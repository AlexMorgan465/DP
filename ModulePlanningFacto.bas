...
'
' 1. Tout le monde (collaborateurs ET manager) a la męme valeur dans "Activité"
...
'
' 7. Cas particuliers pour les agents du projet Facto :
'      - Bara, Dami, El Moubarik, Intaj ont des horaires spécifiques avec rotation.
'      - Leur pause déjeuner est de 30 min (au lieu de 1h).
'
...
Sub GenererPlanningAccessibilite()
    ...
    projectName = InputBox("Nom du projet / de l'activité ŕ générer (ex: Facto) :", _
...
Function ProcessRow(wsBDD As Worksheet, wsPlan As Worksheet, headers As Object, _
    ...
    ' Déterminer la durée de pause pour cet agent (0.5h pour les spéciaux, 1h sinon)
    Dim pauseHours As Double
    pauseHours = GetPauseDuration(nomComplet)
    ...
    For dayIndex = 1 To 7
        ...
        Else
            ...
            ' Utiliser la durée de pause spécifique (0.5 ou 1 heure)
            totalHeures = totalHeures + (sortieH - entreeH - pauseHours)
        End If
...
Function GetDayInfo(wsBDD As Worksheet, headers As Object, rowBDD As Long, _
                     ...
                     nomComplet As String) As Variant
    ...
    ' 4) Horaire (personnalisé pour agents spéciaux, sinon défaut) ---------
    If Not isOff Then
        ' Essayer d'abord les horaires personnalisés pour les 4 agents
        Dim custom As Variant
        custom = GetCustomSchedule(nomComplet, dayIndex)
        If Not IsEmpty(custom) Then
            entreeH = custom(0)
            sortieH = custom(1)
...
Function NormalizeHeader(ByVal s As String) As String
    Dim r As String
    r = UCase(Trim(s))
    r = Replace(r, "É", "E"): r = Replace(r, "Č", "E"): r = Replace(r, "Ę", "E"): r = Replace(r, "Ë", "E")
    r = Replace(r, "Ŕ", "A"): r = Replace(r, "Â", "A")
    r = Replace(r, "Ô", "O")
    r = Replace(r, "Î", "I"): r = Replace(r, "Ď", "I")
    r = Replace(r, "Ů", "U"): r = Replace(r, "Ű", "U")
    r = Replace(r, "Ç", "C")
    r = Replace(r, ".", "")
    r = Replace(r, "'", "")
    r = Replace(r, "-", "")
...
'--------------------------------------------------------------------
' Fonctions pour la gestion des agents spéciaux (Bara, Dami, El Moubarik, Intaj)
'--------------------------------------------------------------------

' Retourne l'identifiant de l'agent (1=Bara, 2=Dami, 3=El Moubarik, 4=Intaj, 0=autre)
Function GetAgentId(ByVal nomComplet As String) As Integer
    Dim n As String
    n = UCase(Trim(nomComplet))
    If InStr(n, "BARA") > 0 Then
        GetAgentId = 1
    ElseIf InStr(n, "DAMI") > 0 Then
        GetAgentId = 2
    ElseIf InStr(n, "EL MOUB") > 0 Or InStr(n, "ELMOU") > 0 Then
        GetAgentId = 3
    ElseIf InStr(n, "INTAJ") > 0 Then
        GetAgentId = 4
    Else
        GetAgentId = 0
    End If
...
' Vérifie si le nom correspond à l'un des quatre agents spéciaux
Function IsSpecialAgent(ByVal nomComplet As String) As Boolean
    IsSpecialAgent = (GetAgentId(nomComplet) > 0)
End Function

' Retourne la durée de pause (en heures) pour l'agent : 0.5h pour les spéciaux, 1h sinon
Function GetPauseDuration(ByVal nomComplet As String) As Double
    If IsSpecialAgent(nomComplet) Then
        GetPauseDuration = 0.5
    Else
        GetPauseDuration = 1
    End If
End Function

' Retourne un tableau (entreeH, sortieH) pour les horaires personnalisés des agents spéciaux.
' Retourne Empty si l'agent n'est pas reconnu ou si le jour est week-end (géré ailleurs).
Function GetCustomSchedule(ByVal nomComplet As String, ByVal dayIndex As Integer) As Variant
    Dim agentId As Integer
    agentId = GetAgentId(nomComplet)
    If agentId = 0 Then
        GetCustomSchedule = Empty
        Exit Function
    End If

    Dim entree As Integer, sortie As Integer
    Select Case agentId
        Case 1 ' Bara
            If dayIndex <= 3 Then
                entree = 7: sortie = 17
    ...
    ' Si dayIndex est en dehors de 1-5 (week-end), on retourne Empty pour que le défaut gère OFF
    If dayIndex >= 1 And dayIndex <= 5 Then
        GetCustomSchedule = Array(entree, sortie)
    Else
        GetCustomSchedule = Empty
    End If
End Function
